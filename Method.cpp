...
cout << "Please enter a seat to reserve (e.g., 3A, 7D, etc.)" << endl;
...
seatchart[reserveRow - 1][tempIndex].setC('#');
...
cout << "Enter a seat that you would like to cancel? (e.g., 3A, 7D, etc.)." << endl;
cin >> allSeatSelection;
tempCin.str(allSeatSelection);
tempCin >> reserveRow;
tempCin >> b;
...
for (int i = 0; i < numColumns; i++) 
{
if (seatchart[reserveRow - 1][i].getC() == '#')
 {
tempIndex = i;
}
}
if (seatchart[reserveRow - 1][tempIndex].getC() == '#') 
{
...
cout << setprecision(3) << reservedSeat / temp * 100 << '%' << " of seats are 
reserved." << endl;
